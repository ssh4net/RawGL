...
std::shared_ptr<GLProgramBuffers> GLProgram::findCounter(std::string name)
{
...
void GLProgram::compileBuffersList()
{
    ...
    struct cntrNames // struct to store atomic counter names and offset in counters_map
    {
        std::string name;
        GLuint offset, size;
		cntrNames(std::string name, GLuint offset, GLuint size) : name(name), offset(offset), size(size) {}
	};
    ...
    struct bufNames // struct to store buffer names and offset in buffers_map
    {
        ...
        GLenum type;
		GLuint offset, size;
		bufNames(std::string name, GLenum type, GLuint offset, GLuint size) : name(name), type(type), offset(offset), size(size) {}
	};
    ...
    struct bufBinding // struct to store buffer binding point
    {
        std::string name;
		GLuint binding, size;
        bufBinding(std::string name, GLuint binding, GLuint size) : name(name), binding(binding), size(size) {}
	};
    ...
    for (int i = 0; i < ncount; i++)
    {
        ...
        const GLenum props[] = {
            GL_TYPE, GL_LOCATION, GL_ARRAY_SIZE, GL_OFFSET, GL_ATOMIC_COUNTER_BUFFER_INDEX,
        ...
        if (size > 1) {
            for (int i = 0; i < size; i++) {
                ...
                for (auto jt = range.first; jt != range.second; ++jt) {
                    buffer_size = size;
                    LOG(trace) << "Counter: " << jt->second.name 
                    << " binding: " << it.first << " offset: " << jt->second.offset << " size: " << jt->second.size << std::endl;
                    
                    m_acounters.insert({
                        jt->second.name, std::make_shared<GLProgramBuffers>(
                            GLProgramBuffers::AtomicCounterBuffer(jt->second.name, map_index, it.first, jt->second.offset, jt->second.size)
                        )
                    });
                    index++;
    ...
    for (int i = 0; i < bncount; i++)
    {
        char name[64];
        GLsizei length;
        const GLenum props[] = { GL_TYPE, GL_ARRAY_SIZE, GL_OFFSET, GL_BLOCK_INDEX,
        ...
        LOG(trace) << "#" << index << " (" << name << ") block index: " << 
            map_index << " offset: " << propData[2] << " size: " << propData[1] << " type: " << glsl_type_name(propData[0]) << std::endl;
    }
    ...
    for (auto it : buffers_bindings) {
        ...
        for (auto jt = range.first; jt != range.second; ++jt) {
            ...
            m_abuffers.insert({
                it.second.name, { jt->second.name, GLProgramBuffers(
                jt->second.name, jt->second.type, it.second.binding, jt->second.offset, jt->second.size)}
			});
...
void GLProgram::compileOutputList()
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        const GLenum props[] = {
            GL_TYPE, GL_LOCATION, GL_ARRAY_SIZE, GL_LOCATION_INDEX, GL_LOCATION_COMPONENT,
            GL_REFERENCED_BY_VERTEX_SHADER, GL_REFERENCED_BY_FRAGMENT_SHADER, GL_REFERENCED_BY_COMPUTE_SHADER };
        GLint propData[8];
        ...
        LOG(trace) << "Output: " << name << " location = " << propData[1] << " type = " << glsl_type_name(propData[0]) << std::endl;
    }
}
...
void GLProgram::DebugShaderVarList()
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        std::cout << "#--------------------------#" << std::endl;
        std::cout << "|    " << propData[5] << "   |    " << propData[6] << "   |    " << propData[7] << "   |" << std::endl;
        std::cout << "#==========================#" << std::endl;
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        GLsizei length;
        const GLenum props[] = {
            GL_TYPE, GL_LOCATION, GL_ARRAY_SIZE, GL_LOCATION_COMPONENT,
            GL_REFERENCED_BY_VERTEX_SHADER, GL_REFERENCED_BY_FRAGMENT_SHADER, GL_REFERENCED_BY_COMPUTE_SHADER };
        GLint propData[7];
        ...
        std::cout << "|   vs   |   fs   |   cs   |" << std::endl;
        std::cout << "#--------------------------#" << std::endl;
        std::cout << "|    " << propData[4] << "   |    " << propData[5] << "   |    " << propData[6] << "   |" << std::endl;
        std::cout << "#==========================#" << std::endl;
    };
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        std::cout << "|   vs   |   fs   |   cs   |" << std::endl;
        std::cout << "#--------------------------#" << std::endl;
        std::cout << "|    " << propData[7] << "   |    " << propData[8] << "   |    " << propData[9] << "   |" << std::endl;
        std::cout << "#==========================#" << std::endl;
    }
