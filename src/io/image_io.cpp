...
OIIO::TypeDesc
image_utils::get_output_format(const std::string& output_filename, const int arg, bool& r_defaulted)
{
    ...
    default: printf("Unknown output image file extension: *.%s\n", output_ext.c_str()); return OIIO::TypeDesc::UINT8;
    }
...
bool
image_utils::progress_callback(void* opaque_data, float portion_done)
{
...
bool
image_utils::load_image(const std::string& filepath, const std::map<std::string, std::string>& attributes, int& width,
                        int& height, void*& pixels, int& channels, int& alphaChannel, OIIO::TypeDesc& format)
{
    ...
    if (in) {
        ...
        if (ext == "crw" || ext == "cs1" || ext == "dc2" || ext == "dcr" || ext == "dng" || ext == "erf" || ext == "fff"
            || ext == "k25" || ext == "kdc" || ext == "mdc" || ext == "mos" || ext == "mrw" || ext == "nef"
            || ext == "orf" || ext == "pef" || ext == "pxn" || ext == "raf" || ext == "raw" || ext == "rdc"
            || ext == "sr2" || ext == "srf" || ext == "x3f" || ext == "arw" || ext == "3fr" || ext == "cine"
            || ext == "ia" || ext == "kc2" || ext == "mef" || ext == "nrw" || ext == "qtk" || ext == "rw2"
            || ext == "sti" || ext == "rwl" || ext == "srw" || ext == "drf" || ext == "dsc" || ext == "ptx"
            || ext == "cap" || ext == "iiq" || ext == "rwz" || ext == "cr3") {
        ...
        case OIIO::TypeDesc::UINT8: bytes = 1; break;
        case OIIO::TypeDesc::UINT16:
        case OIIO::TypeDesc::HALF: bytes = 2; break;
        case OIIO::TypeDesc::UINT32:
        case OIIO::TypeDesc::FLOAT: bytes = 4; break;
        case OIIO::TypeDesc::UINT64:
        case OIIO::TypeDesc::DOUBLE: bytes = 8; break;
        default:
        ...
        if (in->read_image(0, 0, 0, channels, format, pixels, OIIO::AutoStride, OIIO::AutoStride, OIIO::AutoStride,
                           (*progress_callback))) {
            if (in->close()) {
